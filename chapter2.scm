...
(define (install-dense-termlist-package)
  ...
  (define (order termlist) (car termlist))
  (define (coeff termlist) (cdr termlist))
  
  (define the-empty-termlist '())
  (define (empty-termlist? termlist) (null? termlist))
  (define (first-term termlist) (make-term (- (length termlist) 1)
					   (car termlist)))
  (define (rest-terms termlist) (cdr termlist))
  (define (adjoin-term term termlist)
    (cond ((=zero? (coeff term)) termlist)
	  ((= (order term) (length termlist))
	      (cons (coeff term) termlist))
	  ((> (order term) (length termlist))
	   (adjoin-term term
			(cons 0 termlist)))
	  (else
	   (error "ADJOIN_TERM" "Alreay has a term with the same order in the termlist" term termlist))))
  (define (add-terms L1 L2)
    (cond ((empty-termlist? L1) L2)
	  ((empty-termlist? L2) L1)	  
	  (else
  ...
  (define (mul-terms L1 L2)
	...
	((the-empty-termlist)
	 (let ((first (first-term termlist)))
	   (add-terms (mul-one-term-to-all first L2)
		      (mul-terms (rest-terms L1) L2))))))
  (define (mul-one-term-to-all term termlist)
    (if (empty-termlist? termlist)
	(the-empty-termlist)
	(let ((first (first-term termlist)))
	  (adjoin-term (make-term (+ (order term) (order first))
				  (mul (coeff term) (coeff first)))
		       (mul-one-term-to-all term (rest-terms termlist))))))
  (define (neg-terms termlist)
    (if (empty-termlist? termlist)
	(the-empty-termlist)
...
(define (install-sparse-polynomial-package)
  ...
  (put 'neg '(sparse-polynomial)
       (lambda (p) (tag (neg-poly p))))
  (put '=zero? '(sparse-polynomial) =poly-zero?)
...
(define (install-dense-polynomial-package)
  ...
  (put 'mul '(dense-polynomial dense-polynomial)
       (lambda (p1 p2) (tag (mul-poly p1 p2))))
  (put 'neg '(dense-polynomial)
       (lambda (p) (tag (neg-poly p))))
  (put '=zero? '(dense-polynomial) =poly-zero?)
  ...
  (define (add-rat x y)
    (make-rat (add (mul (numer x) (denom y))
		 (mul (numer y) (denom x)))
	      (mul (denom x) (denom y))))
  (define (sub-rat x y)
    (make-rat (sub (mul (numer x) (numer y))
		 (mul (numer y) (numer x)))
	      (mul (denom x) (denom y))))
  (define (mul-rat x y)
